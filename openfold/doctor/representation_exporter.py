...
class RepresentationExporter:
    ...
    def _representation_hook(self, msa_representation, pair_representation, stage, iteration):
        if self.export_msa:
            self._save_heatmap(msa_representation, stage, iteration, "msa")

        if self.export_pair:
            self._save_heatmap(pair_representation, stage, iteration, "pair")

    def _save_heatmap(self, data, stage, iteration, which):
        if data is None:
